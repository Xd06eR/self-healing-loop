...
- **Leave every existing test exactly as it is** — the frozen reproducing test the driver names, and every other one. No removed assertions, no skip or expected-failure marker by any mechanism this language offers, no renaming a file to dodge it. Adding a test is the change that is welcome.
- **Leave test-runner configuration alone.** Find what this project actually uses — the file configuring the runner, the file listing which paths it collects, the manifest carrying the test script — and do not edit it. The gate rejects any diff that touches one.
