...
- `SHL_MODEL` sets `ANTHROPIC_MODEL` **and every model alias the harness resolves**, including each tier and the subagent model. A third-party endpoint serves one catalogue while the harness still resolves its own aliases internally, so any alias left at its default names a model that provider has never heard of. The failure lands mid-cycle rather than at startup, which reads as an unreliable agent instead of a missing variable.
