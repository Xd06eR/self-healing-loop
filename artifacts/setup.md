...
- [ ] **Must-pass: prove the gate BLOCKS a weakened test.** There is no way to inject a fix into a cycle, because the agent authors it, so run the gate against a weakened diff directly. Delete an assertion from any test file, then:
