...
Design, mechanics and the incidents behind these rules live in the framework's own `CLAUDE.md`, beside this file. Not the target's `CLAUDE.md`, and not `loop_context/CLAUDE.md`, which is copied in for the loop's agent.
...
4. *(2 = host logs, **or** 3 = generate a relay)* **What is the log retention, and which plan tier sets it?** Ask the tier, look the value up in the platform's own docs, show the number to confirm. Record as `{{RETENTION}}` and set the cron from it (`{{CRON}}`). Wrong here loses failures silently, and idle looks healthy. Mechanics, including why the relay branch needs this too: [reference/platforms.md](reference/platforms.md).
